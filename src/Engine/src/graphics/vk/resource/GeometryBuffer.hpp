...
{
public:
    GeometryBuffer(std::uint64_t size);
    ...
    [[nodiscard]] std::uint64_t queue_transfer(buffer_t&& data /*, CUploadContext& uploadContext*/)
    {
        ODIN_ASSERT(!data.empty());

        std::size_t bytesToWrite = data.size() * sizeof(typename std::remove_cvref_t<buffer_t>::element_type);
        std::uint64_t bytesRemain = m_BufferSize - m_TransferOffset;
        ...
        m_TransferOffset += data.size();

        return m_TransferOffset;
    };
private:
    std::uint64_t m_BufferSize;
    std::uint64_t m_TransferOffset;
};
}    // namespace odin::graphics::vk::resource
