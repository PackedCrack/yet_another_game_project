...
[[nodiscard]] VmaAllocator make_allocator(const odin::graphics::vk::Instance& instance, const odin::graphics::vk::PhysicalDevice& gpu, const odin::graphics::vk::Device& device)
{
	using namespace odin::graphics;

	VmaAllocatorCreateInfo info{};

	vk::InstanceView inst = instance.view();
	info.instance = inst.handle;
	info.physicalDevice = gpu.handle();
	
	vk::DeviceView dev = device.view();
...
{
public:
	Impl(const vk::Instance& instance, const vk::PhysicalDevice& gpu, const vk::Device& device)
...
public:
	vk::Image create_image(std::shared_ptr<Allocator> pAllocator, VmaAllocationCreateInfo allocInfo, const VkImageCreateInfo& imageInfo)
	{
		VkImage image{};
		...
		return vk::Image{ std::move(pAllocator), allocImage, imageInfo.format };
	}
...
Allocator::Allocator(const vk::Instance& instance, const vk::PhysicalDevice& gpu, const vk::Device& device)
	: m_pImpl{ std::make_unique<Allocator::Impl>(instance, gpu, device) }
{}
[[nodiscard]] vk::Image Allocator::create_image_cpu_only(const VkImageCreateInfo& info)
{
...
[[nodiscard]] vk::Image Allocator::create_image_gpu_only(const VkImageCreateInfo& info)
{
	VmaAllocationCreateInfo allocInfo = make_allocation_create_info(VMA_MEMORY_USAGE_GPU_ONLY);
	return m_pImpl->create_image(shared_from_this(), allocInfo, info);
}
[[nodiscard]] vk::Image Allocator::create_image_cpu_to_gpu(const VkImageCreateInfo& info)
{
